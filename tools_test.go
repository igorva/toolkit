	...
	for _, e := range uploadTests {
		...
		if e.errorExpected {
			if err == nil {
				...
				t.Logf("%s: error expected and received: %s", e.name, err)
			}
		}
...
var jsonTests = []struct {
	...
	{name: "two json files", json: `{"foo": "bar"} {"alpha":"betta"}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "empty body", json: ``, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "syntax error in json", json: `{"foo": "bar}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "unknown field in json", json: `{"alpha":"betta"}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "allow unknown fields in json", json: `{"alpha":"betta"}`, errorExpected: false, maxSize: 1024, allowUnknown: true},
	{name: "missing field name", json: `{alpha:"betta"}`, errorExpected: true, maxSize: 1024, allowUnknown: true},
...
func TestTools_WriteJSON(t *testing.T) {
	...
	if err != nil {
		t.Errorf("failed to write JSON: %s\n", err)
	}
}
...
func TestTools_ErrorJSON(t *testing.T) {
	...
	if err != nil {
		t.Error("received error wen decoding JSON", err)
	}
	...
	if rr.Code != http.StatusServiceUnavailable {
		t.Errorf("wrong status code returned; expected 503, but got %d\n", rr.Code)
	}
}
